...
ARCHIVE_NAME="frontend_ng_${TIMESTAMP}.tar.gz"

echo "🗜️ Comprimiendo archivos..."
cd dist
tar -czf "../${ARCHIVE_NAME}" frontend_ng/
cd ..
...
ssh -i "${KEY_PATH}" \
    ...
    if [ -d "/var/www/html/frontend_ng" ]; then
        sudo mv /var/www/html/frontend_ng /var/www/html/backups/frontend_ng_backup_${TIMESTAMP}
        echo "✅ Backup creado: frontend_ng_backup_${TIMESTAMP}"
    fi
    
    echo "📁 Creando directorio web..."
    sudo mkdir -p /var/www/html/frontend_ng
    
    echo "📁 Moviendo nueva versión..."
    sudo cp -r frontend_ng/* /var/www/html/frontend_ng/
    
    echo "🔐 Configurando permisos..."
    sudo chown -R www-data:www-data /var/www/html/frontend_ng
    sudo chmod -R 755 /var/www/html/frontend_ng
    
    echo "🔍 Verificando archivos copiados..."
    ls -la /var/www/html/frontend_ng/
